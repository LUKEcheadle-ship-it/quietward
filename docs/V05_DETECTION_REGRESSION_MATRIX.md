...
| PROC-08 | Web/server parent spawns already-suspicious shell child | `web_server_spawned_suspicious_shell`, HIGH+ | v0.5 hardening tests/source gate |
| PROC-09 | Web/server parent spawns normal maintenance shell | no ancestry marker by itself | v0.5 hardening tests/source gate |
| IMPACT-01 | `vssadmin(.exe) delete shadows` | recovery-inhibition marker, HIGH+ | `test_windows_impact_evasion_v05.py` / adversarial pack |
| IMPACT-02 | `wmic(.exe) shadowcopy delete` | recovery-inhibition marker, HIGH+ | `test_windows_impact_evasion_v05.py` / adversarial pack |
...
| EVADE-01 | explicit event-log clearing | `event_log_clearing`, HIGH+ | `test_windows_impact_evasion_v05.py` / adversarial pack |
...
| CONT-01 | Docker socket or host-root exposure | high-confidence container security evidence | scoring/hardening tests |
| CONT-02 | weak but common container context such as missing no-new-privileges alone | no automatic HIGH behavior floor | adversarial scoring pack |
...
| CHAIN-08 | events outside 15-minute chain window | no chain | hardening/adversarial tests |
| CHAIN-09 | suspicious process name aligns with adjacent network process | process/network corroboration bonus | v0.5 correlation tests/source gate |
| PRIV-01 | command arguments persisted raw | forbidden; only hashes + bounded markers | parser/privacy regressions |
